...
function generateAllOptions($rootScope){
	var childDE = $rootScope.selectedInstanceDEs;
	var isSelected = "";
	var allOptions = "";
	...
	childDE.forEach(function(cDE) {
		if( !cDE.hasOwnProperty('categoryCombo') )//indicators
		{
			...
			allOptions += "<option value="+cDE.id+" "+isSelected+">";
			allOptions += cDE.name + " -- (Indicator)";
			allOptions += "</option>";
		...
		else if(cDE.categoryCombo.categoryOptionCombos.length > 0)
		{
			cDE.categoryCombo.categoryOptionCombos.forEach(function(ccoc) {
				...
				allOptions += "<option value="+optionValue+" "+isSelected+">";
				allOptions += cDE.name + " - " + ccoc.name;
				allOptions += "</option>";
